...
import { TablesInsert } from "@/database.types"; // Adjust the path as needed
...
const AddNannyForm: React.FC = () => {
  ...
  const onSubmit = async (data: BioFormValues) => {
    try {
      // 1. Upload selected images to the "profile_pictures" bucket
      const imageFiles: File[] = data.images;
      const imageUrls = await uploadImages(imageFiles, "profile_pictures");
      if (imageUrls.length === 0) throw new Error("Image upload failed");
      const avatarUrl = imageUrls[0];

      // 2. Retrieve existing user ID from sessionStorage or generate a new one
      let userId = sessionStorage.getItem("nannyUserId");
      if (!userId) {
        userId = crypto.randomUUID();
      }

      // 3. Build the upsert payload ensuring the correct types (using our generated types)
      const payload: TablesInsert<"user_accounts"> = {
        id: userId,
      ...
      // 4. Upsert the record in the user_accounts table and chain .select() to get the record
      ...
      // 5. Save the user ID and bio data for subsequent steps
      sessionStorage.setItem("nannyUserId", userId);
  ...
  return (
    <div className="min-h-screen overflow-y-auto bg-white">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 py-6">
        ...
        <div className="w-full p-3 sm:w-[70%] md:w-[80%] mx-auto">
          <div>
            <p className="mb-2 font-barlow text-base font-semibold">
              Edit Bio Information
            </p>
            <label
              htmlFor="full_name"
              className="block font-barlow text-sm font-medium text-gray-700"
            >
              Full Name
            </label>
            <input
          ...
          <div className="mt-5 flex items-center justify-center md:justify-end gap-3">
            ...
            <button
              type="submit"
              className="px-8 py-3 bg-[#6000DA] rounded-lg text-white"
            >
              Proceed
            </button>
          </div>
