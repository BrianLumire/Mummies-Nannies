...
import { NannyData } from "@/hooks/nanny/useNannies"; // Import the correct type

interface Column {
  header: string;
  accessor: keyof NannyData; // Ensure accessor matches NannyData keys
}

interface NannyTableProps {
  columns: Column[];
  data: NannyData[];
  renderRow: (item: NannyData) => React.ReactNode;
  searchTerm: string;
